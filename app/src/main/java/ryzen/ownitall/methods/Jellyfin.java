...
public class Jellyfin {
    ...
    private void authenticate() {
        ...
        JsonNode response = this.query("post", "/Users/AuthenticateByName", credsNode);
        if (response == null) {
    ...
    public void uploadLikedSongs(ArrayList<Song> songs) {
        ...
        if (!songIds.isEmpty()) {
            for (String songId : songIds) {
                this.query("post", "/UserFavoriteItems/" + songId, null);
            }
        }
    }
    ...
    public String getSongId(Song song) {
        ...
        if (song.getArtist() != null) {
            params.putArray("artists").add(song.getArtist().getName());
        }
        if (song.getAlbumName() != null) {
            params.putArray("albums").add(song.getAlbumName());
        }
        params.putArray("mediaTypes").add("Audio");
        params.put("recursive", true);
        params.put("limit", 1);
        JsonNode response = this.query("get", "/Items", params);
        if (response != null) {
    ...
    private JsonNode query(String method, String type, JsonNode params) {
        if (method == null || method.isEmpty()) {
            logger.debug("null or invalid method provided in query");
            return null;
        }
        if (params == null || params.isEmpty()) {
            logger.debug("null or invalid parameters provided in query");
            return null;
        }
        try {
